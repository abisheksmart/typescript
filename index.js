...
btn1?.addEventListener("click", function (e) {
  ...
  if (pass1.length >= 8 && name1.length > 0) {
    if (
      pass1.match(isUpperCase) &&
      pass1.match(isLowerCase) &&
      pass1.match(isSpecialChar) &&
      pass1.match(isNumeric)
    ) {
      ...
      err.textContent =
        "Password Must contain 1 Uppercase,1 Lowercase, 1 Special Character & 1 Numeric Value";
    }
  } else {
    err.textContent =
      "Fields cann't be empty and Length of password must be greater than 7";
  }
});
